...
This package provides scripts used in the Office demo to demonstrate the tasks that may be requested of a robot.
...
- **dispatch_patrol**
  ...
  ```
  ros2 run office_tasks dispatch_patrol -F tinyRobot -R tinyRobot1 -p lounge -n 2 --use_sim_time
  ```

  or multiple places for the robot to travel to:
  ```
  ros2 run office_tasks dispatch_patrol -p supplies pantry coe --use_sim_time
  ```
...
- **dispatch_clean**
  ...
  ```
  ros2 run office_tasks dispatch_clean -cs clean_lobby -F cleanerBotA -R cleanerBotA_1 --use_sim_time
  ```
...
- **dispatch_action**
  ...
  ```
  ros2 run office_tasks dispatch_action -F tinyRobot -R tinyRobot1 -a teleop -s coe --use_sim_time
  ```

  After completing your teleop action, publish the following message to return control to the fleet adapter:
  ```
  ...
  ```
  ros2 run office_tasks dispatch_action -a teleop -s coe supplies pantry --use_sim_time
  ```

  The robot will move to each of these places and the fleet adapter will relinquish control. Similar to `dispatch_action` earlier, you may perform your desired action (e.g. using the provided `teleop_robot` script) and end the action by publishing a `ModeRequest` to `/action_execution_notice`. The robot will then move to the next point to perform the next action.
...
- **dispatch_delivery**
  ...
  ```bash
  ros2 run office_tasks dispatch_delivery \
  -p pantry pantry \
...
- **cancel_task**
  ...
  ```
  ros2 run office_tasks dispatch_patrol -p coe lounge --use_sim_time
  ```

  Then try cancel the submitted task with the id.
  ```bash
  ros2 run office_tasks cancel_task -id patrol.dispatch-0
  ```
  ...
  **cancel_robot_task**
  ...
  Example: Run dispatch a `patrol` task to the newly launched `office_world`.
  ```
  ros2 run office_tasks dispatch_go_to_place -p pantry -F tinyRobot -R tinyRobot2 --use_sim_time
  ```

  Then try cancel the submitted task with the id.
  ```bash
  ros2 run office_tasks cancel_robot_task -F tinyRobot -R tinyRobot2
  ```
...
- **office_teleop_robot**
  ...
  ```
  ros2 launch office_demos office_teleop_robot.launch.xml
  ```
...
- **dispatch_go_to_place**
  ...
  ```
  ros2 run office_tasks  dispatch_go_to_place -p lounge -o 105 --use_sim_time
  ```
...
- **wait_for_task_complete**
  ...
  ```
  ros2 run office_tasks wait_for_task_complete -F tinyRobot -R tinyRobot1
  ```
