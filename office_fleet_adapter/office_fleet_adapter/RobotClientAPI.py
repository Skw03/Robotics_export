...
class RobotAPI:
    ...
    def navigate(self,
        ...
        assert(len(pose) > 2)
        url = self.prefix +\
            f'/open-rmf/office_demos_fm/navigate/?robot_name={robot_name}' \
            f'&cmd_id={cmd_id}'
    ...
    def start_process(self,
        ...
        url = self.prefix +\
            f"/open-rmf/office_demos_fm/start_task?robot_name={robot_name}" \
            f"&cmd_id={cmd_id}"
    ...
    def toggle_action(self, robot_name: str, toggle: bool):
        ...
        url = self.prefix +\
            f"/open-rmf/office_demos_fm/toggle_action?robot_name={robot_name}"
        data = {'toggle': toggle}
        try:
            response = requests.post(url, timeout=self.timeout, json=data)
            response.raise_for_status()
